...
/* ═══════════════════════════════════════════════════════
   RippleGrid — WebGL animated grid background
   ─────────────────────────────────────────────────────
   Uses OGL to render a rippling grid effect with:
   • Configurable grid color, size, and thickness
   • Sine-wave ripple distortion
   • Mouse interaction (radial wave from cursor)
   • Vignette fade-out toward edges
   • Optional rainbow mode
   
   Props are reactive — changing them updates the shader
   uniforms in real-time without re-creating the WebGL
   context.
   ═══════════════════════════════════════════════════════ */
...
const RippleGrid = ({
    ...
    useEffect(() => {
        ...
        /* ── Vertex shader ──
           Simple full-screen triangle passthrough */
        const vert = `
...
void main() {
    ...
    gl_Position = vec4(position, 0.0, 1.0);
        ...
        /* ── Fragment shader ──
           Renders the animated grid with ripple distortion,
           mouse interaction, vignette, and optional rainbow */
...
void main() {
    ...
    /* Optional grid rotation */
    if (gridRotation != 0.0) {
        uv = rotate(gridRotation * pi / 180.0) * uv;
    }

    /* Base ripple distortion — sine wave radiating from center */
    float dist = length(uv);
    float func = sin(pi * (iTime - dist));
    vec2 rippleUv = uv + uv * func * rippleIntensity;

    /* Mouse-driven secondary ripple */
    ...
    /* Grid line rendering with anti-aliasing */
    ...
    /* Accumulate grid line intensity */
    vec3 color = vec3(0.0);
    ...
    /* Optional glow around grid lines */
    if (glowIntensity > 0.0) {
        color += glowIntensity * exp(-gridThickness * 0.5 * smoothB.x);
        color += glowIntensity * exp(-gridThickness * 0.5 * smoothB.y);
    }

    /* Distance-based radial fade */
    ...
    /* ── Reactive prop updates ──
       When props change, update the shader uniforms
       in-place without re-creating the WebGL context */
    useEffect(() => {
