...
/* ═══════════════════════════════════════════════════════
   LiquidEther Engine — createLiquidEther factory
   -------------------------------------------------------
   All classes extracted to module scope to satisfy
   React compiler rules (no inline class declarations).
   
   Returns a manager object with start/pause/resize/dispose.
   ═══════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════
   CommonClass — shared renderer + sizing
   ══════════════════════════════════════════ */
class CommonClass {
  constructor() {
    this.width = 0;
...
/* ══════════════════════════════════════════
   MouseClass — tracks pointer within container
   ══════════════════════════════════════════ */
class MouseClass {
  constructor() {
...
/* ══════════════════════════════════════════
   AutoDriver — generates idle cursor movement
   ══════════════════════════════════════════ */
class AutoDriver {
...
/* ══════════════════════════════════════════
   ShaderPass — base class for GPU passes
   ══════════════════════════════════════════ */
class ShaderPass {
...
/* ══════════════════════════════════════════
   Simulation — fluid solver pipeline
   ══════════════════════════════════════════ */
class Simulation {
...
/* ══════════════════════════════════════════
   Output — final compositing pass
   ══════════════════════════════════════════ */
class Output {
  constructor(common, mouse, paletteTex, bgVec4, simOptions) {
    this.common = common;
...
/* ═══════════════════════════════════════════════════════
   createLiquidEther — factory function
   -------------------------------------------------------
   Called once in useEffect. Returns { start, pause, resize, dispose }.
   Manages WebGL lifecycle, IntersectionObserver, etc.
   ═══════════════════════════════════════════════════════ */
export function createLiquidEther(container, config, callbacks) {
  const { isVisibleRef, rafRef } = callbacks;

  const isMobile = window.innerWidth <= (config.mobileBreakpoint || 768);
  const activeResolution = isMobile
    ? config.mobileResolution || 0.4
    : config.resolution;
  ...
  const managerLike = { lastUserInteraction };
  // Keep reference current for AutoDriver
  Object.defineProperty(managerLike, "lastUserInteraction", {
    get: () => lastUserInteraction,
    set: (v) => {
      lastUserInteraction = v;
    },
  });

  const autoDriver = new AutoDriver(mouse, managerLike, {
  ...
  const output = new Output(common, mouse, paletteTex, bgVec4, simOptions);
  ...
  return { start, pause, resize, dispose };
}
